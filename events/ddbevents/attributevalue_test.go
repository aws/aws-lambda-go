...
func TestAccessWithWrongTypePanics(t *testing.T) {
	testCases := []struct {
		input    string
		accessor func(av AttributeValue)
	}{
		{`{ "B": "AAEqQQ=="}`, func(av AttributeValue) { av.Number() }},
		{`{ "BOOL": true}`, func(av AttributeValue) { av.Number() }},
		{`{ "BS": ["AAEqQQ==", "AAEqQQ=="] }`, func(av AttributeValue) { av.Number() }},
		{`{ "L": [ { "S": "Cookies"} ] }`, func(av AttributeValue) { av.Number() }},
		{`{ "M": { "Name": { "S": "Joe" } } }`, func(av AttributeValue) { av.Number() }},
		{`{ "N": "123.45"}`, func(av AttributeValue) { av.Boolean() }},
		{`{ "NS": ["1234", "567.8"] }`, func(av AttributeValue) { av.Boolean() }},
		{`{ "NULL": true}`, func(av AttributeValue) { av.Number() }},
		{`{ "S": "Hello"}`, func(av AttributeValue) { av.Number() }},
		{`{ "SS": [ "Giraffe", "Zebra" ] }`, func(av AttributeValue) { av.Number() }},
	}
	...
	for _, testCase := range testCases {
		...
		assert.Nil(t, err)
		// may use PanicsWithValue(ErrInvalidType) when it is available
		assertPanicsWithValue(t, ErrInvalidType, func() { testCase.accessor(av) })
	}
}
